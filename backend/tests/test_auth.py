...
async def test_register_duplicate_email_silent(client: AsyncClient, db_session):
    ...
    user = User(id=uuid.uuid4(), email="dup@test.com", password_hash=hash_password("password123"), is_verified=True)
    db_session.add(user)
...
async def test_login(client: AsyncClient, db_session):
    ...
    user = User(id=uuid.uuid4(), email="login@test.com", password_hash=hash_password("password123456"), is_verified=True)
    db_session.add(user)
    await db_session.commit()
...
async def test_login_unverified_email(client: AsyncClient, db_session):
    ...
    user = User(id=uuid.uuid4(), email="unverified@test.com", password_hash=hash_password("password123456"), is_verified=False)
    db_session.add(user)
    await db_session.commit()
...
async def test_refresh_token(client: AsyncClient, db_session):
    ...
    user = User(id=uuid.uuid4(), email="refresh@test.com", password_hash=hash_password("password123456"), is_verified=True)
    db_session.add(user)

    refresh_str = create_refresh_token(user.id)
    rt = RefreshToken(id=uuid.uuid4(), user_id=user.id, token_hash=hash_token(refresh_str), expires_at=datetime.now(timezone.utc) + timedelta(days=7))
    db_session.add(rt)
    await db_session.commit()
...
async def test_refresh_token_reuse_revokes_all(client: AsyncClient, db_session):
    ...
    user = User(id=uuid.uuid4(), email="reuse@test.com", password_hash=hash_password("password123456"), is_verified=True)
    db_session.add(user)
...
async def test_logout(client: AsyncClient, db_session):
    ...
    user = User(id=uuid.uuid4(), email="logout@test.com", password_hash=hash_password("password123456"), is_verified=True)
    db_session.add(user)
...
async def test_forgot_password(client: AsyncClient, db_session):
    ...
    user = User(id=uuid.uuid4(), email="forgot@test.com", password_hash=hash_password("password123456"), is_verified=True)
    db_session.add(user)
    await db_session.commit()
...
async def test_change_password(client: AsyncClient, db_session):
    ...
    user = User(id=uuid.uuid4(), email="changepw@test.com", password_hash=hash_password("oldpassword12345"), is_verified=True)
    db_session.add(user)
    await db_session.commit()
    ...
    resp = await client.post("/api/users/me/change-password", json={
...
async def test_delete_account(client: AsyncClient, db_session):
    ...
    user = User(id=uuid.uuid4(), email="delete@test.com", password_hash=hash_password("testpassword123"), is_verified=True)
    db_session.add(user)
    await db_session.commit()
